...
export function RoundsTab({
    ...
    onAutoFillByAdp: (maxPerRound: number) => void
    autoFillAvailableCount: number
    ...
    const [maxPerRound, setMaxPerRound] = useState(1)
            ...
            <div className="flex items-center justify-between border-b px-3 py-2">
                ...
                <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1.5">
                        <Label htmlFor="auto-fill-max" className="text-xs text-muted-foreground">
                            Max per round
                        </Label>
                        <Input
                            id="auto-fill-max"
                            ...
                            min={1}
                            value={maxPerRound}
                            onChange={(e) => setMaxPerRound(Math.max(1, Number(e.target.value) || 1))}
                            className="h-8 w-16"
                        />
                    </div>
                    <Button
                        variant="outline"
                        size="sm"
                        disabled={autoFillAvailableCount === 0}
                        onClick={() => onAutoFillByAdp(maxPerRound)}
                    >
                        Auto-Fill by ADP
                    </Button>
                    <AlertDialog open={resetDialogOpen} onOpenChange={setResetDialogOpen}>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    Every player placed in a round will be sent back to the pool. This can&apos;t be
                                    undone automatically, though nothing is deleted - you can re-place players from
                                    the pool.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
